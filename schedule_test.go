...
func TestNewAppointment(t *testing.T) {
	...
	t.Run("Add new invalid appointment to a schedule", func(t *testing.T) {
		schedule := newTestSchedule(baseDateTime)
...
func TestNewSpecialAppointment(t *testing.T) {
	baseDateTime := time.Date(2021, time.May, 13, 14, 5, 0, 0, time.UTC)

	t.Run("Add new valid special appointment to a schedule", func(t *testing.T) {
		schedule := newTestSchedule(baseDateTime)

		newAppointment, err := schedule.NewSpecialAppointment(baseDateTime)
